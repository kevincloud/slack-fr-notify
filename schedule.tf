resource "aws_scheduler_schedule" "fr_schedule" {
  name       = "${var.unique_identifier}-frnotify-schedule"
  group_name = aws_scheduler_schedule_group.fr_schedule_group.name
  ...
  target {
    arn      = aws_lambda_function.lambda_notifyapp.arn
    role_arn = aws_iam_role.schedule_iam_role.arn
  }
}

resource "aws_scheduler_schedule_group" "fr_schedule_group" {
